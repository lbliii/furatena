...
Statically recoverable messages in public functions must end with punctuation
and contain at least eight words. Private helpers, bare re-raises, and dynamic
messages remain outside this syntactic gate. The committed baseline records
existing debt; CI rejects new debt and stale fingerprints after a fix.
"""
...
SRC_ROOT = ROOT / "src" / "furatena"
DEFAULT_BASELINE = ROOT / "docs" / "raise-message-baseline.txt"
...
def _write_baseline(path: Path, violations: list[Violation]) -> None:
    ...
    content = [
        "# Furatena raise-message debt baseline. See scripts/lint_raise_messages.py.",
...
def _report_drift(violations: list[Violation], baseline: Counter[str]) -> int:
    ...
    if added:
        ...
        for fingerprint in sorted(added):
            for violation in by_fingerprint[fingerprint][: added[fingerprint]]:
                print(f"  {violation.render()}", file=sys.stderr)
    if resolved:
        print(
            f"{sum(resolved.values())} baseline finding(s) were resolved; remove their fingerprints.",
            file=sys.stderr,
        )
    print(
        "Review changes, then regenerate deliberately with "
        "--write-baseline docs/raise-message-baseline.txt.",
        file=sys.stderr,
    )
    return 1
