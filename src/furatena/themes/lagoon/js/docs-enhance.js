 ...
 * - Entry point: FuraDocs.enhance.refresh(#page-root)
 */
(function () {
  'use strict';
  ...
  function cleanPrintUrl(raw) {
    ...
    Array.from(parsed.searchParams.keys()).forEach(function (key) {
      ...
      if (normalized.indexOf('utm_') === 0 || normalized === 'fbclid' || normalized === 'gclid') {
  ...
  function preparePrintPublication() {
    if (printState) return;
    var root = document.documentElement;
    printState = {
      hadTheme: root.hasAttribute('data-theme'),
    ...
    document.querySelectorAll('a[href]').forEach(function (link) {
      ...
      var cleaned = cleanPrintUrl(original);
      if (cleaned === original) return;
      printState.links.push({ link: link, href: original });
      link.setAttribute('href', cleaned);
    });
  }

  function restorePrintPublication() {
    if (!printState) return;
    var root = document.documentElement;
    printState.links.forEach(function (record) {
      record.link.setAttribute('href', record.href);
    });
    if (printState.hadTheme) {
      root.setAttribute('data-theme', printState.theme);
    } else {
  ...
  window.addEventListener('beforeprint', preparePrintPublication);
  window.addEventListener('afterprint', restorePrintPublication);
  ...
  var copyToClipboard =
    (window.FuraDocsUtils && window.FuraDocsUtils.copyToClipboard) ||
    (async function (text) {
  ...
  register('heading-anchors', {
    enhance: function (root) {
      var copyToClipboardFn =
        (window.FuraDocsUtils && window.FuraDocsUtils.copyToClipboard) || copyToClipboard;
      var scopes = root.querySelectorAll(
        '.chirp-theme-docs-layout__content, .chirp-theme-track-section__body, .chirp-theme-page__content, .chirp-theme-home__content, .docs-content'
      );
      if (!scopes.length) {
        return;
      }
  ...
  register('toc', {
    enhance: function (root) {
      if (window.FuraDocsTOC) {
        window.FuraDocsTOC.init(root);
      }
    },
    cleanup: function (root) {
      if (window.FuraDocsTOC) {
        window.FuraDocsTOC.cleanup(root);
      }
    },
  });
  ...
  register('docs-nav', {
    enhance: function (root) {
      if (window.FuraDocsNav) {
        window.FuraDocsNav.enhance(root);
      }
    },
  });

  window.FuraDocs = window.FuraDocs || {};
  window.FuraDocs.enhance = {
    register: register,
    refresh: refresh,
    cleanup: cleanup,
    enhance: enhance,
  ...
  window.FuraDocsChrome = { refresh: refresh };

  window.furaDocsTabSet = function (initial, syncKey, tabIds) {
    tabIds = tabIds || [];
  ...
  (function bootstrapEnhancements() {
    ...
    if (window.FuraDocsUtils && window.FuraDocsUtils.ready) {
      window.FuraDocsUtils.ready(run);
