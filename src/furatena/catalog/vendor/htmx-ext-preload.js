(function() {
  ...
  */
  ...
  function sendHxGetRequest(url, sourceNode, formData = undefined) {
    // Speculative requests must not participate in the link's navigation
    // sync, or an in-flight hover request can replace the user's click.
    const sync = sourceNode.getAttribute('hx-sync')
    if (sync !== null) {
      sourceNode.removeAttribute('hx-sync')
    }
    try {
      htmx.ajax('GET', url, {
        source: sourceNode,
      ...
      if (sync !== null) {
        sourceNode.setAttribute('hx-sync', sync)
      }
    }
  }
