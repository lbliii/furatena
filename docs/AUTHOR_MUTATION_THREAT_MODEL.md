...
|---------|-------------|------------------|---------------------------|---------------------|--------------------|--------------------|
| Browser author chrome | `GET /docs/_author/transition` | `draft`, `publish`, `unpublish`, `archive` | Author mode only | Query-string `confirmed`; no valid CSRF token | Operation id and diff response; Git/manual rollback | #100, #151 |
| Browser author studio | `POST /docs/_author/studio/save` | Create draft, replace full source | Author mode only | Form submission; no valid CSRF token | Operation result and reindex; Git/manual rollback | #100, #151, #197 |
