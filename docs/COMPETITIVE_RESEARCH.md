...
| Release and installation | **Not product-ready** | Package metadata says `0.1.0` and Alpha, requires CPython `>=3.14,<3.15`, and operationally requires free-threaded 3.14 with the GIL disabled. The first tag workflow failed during provenance attestation because the repository is private; no release was published. |
