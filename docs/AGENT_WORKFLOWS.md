...
The server exposes catalog nodes, the DCP catalog graph, API/autodoc operations, structure indexes, inventories, source health, channel manifests, validation reports, and stale-impact reports as JSON resources. Operators should use `/healthz` only for process liveness and `/readyz` for safe-to-serve admission; `/catalog/operational-status.json` adds source/index freshness and freeze/export artifact ages.
...
The `fura://reports/audit` resource records sanitized tool calls with actor, tenant, site, tool name, redacted inputs, result status, duration, and the configured timeout. Authoring entries also include command, target path, previous/resulting state, diagnostics, dry-run state, and confirmation state for reviewable mutation trails. `--rate-limit`, `--tenant-rate-limit`, `--rate-limit-burst`, and `--sensitive-rate-limit` define actor, tenant, burst, and sensitive-tool bounds. Add `--rate-limit-store PATH` for restart-safe multi-worker SQLite counters; configured shared-store failures deny by default, while `--rate-limit-fallback memory` explicitly accepts process-local bypass risk. `--timeout` and `--max-output-chars` define timeout metadata and output truncation bounds for local and remote transports.
