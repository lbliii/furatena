...
- `fura author new|status|validate|inspect-public|edit|draft|publish|unpublish|archive --json`
...
`fura author inspect-public TARGET --operation publish|unpublish|archive --json` simulates every anonymous HTML, navigation, search, DCP, static, sitemap, PDF, text, Markdown, LLM, tools, channel, and MCP projection. It is bound to the exact source and projection-plan digests and never mutates source or delivery state.
