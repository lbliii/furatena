...
|---|---|---|
| CLI | `cli_command` | 32 commands; all identifier-linked |
| Routes | `route` | 46 routes; 38 linked, 8 missing |
...
| Diagnostics | `diagnostic` | 46 rule ids/families; all identifier-linked |
...
| `solo-founder` | `routes` | Intentionally internal — route registration is not an onboarding goal | Partial — navigation tasks omit recovery from dead ends | Existing — [Hypermedia model](/docs/concepts/hypermedia-model/) explains route behavior | Partial — 8 browser-oriented routes still lack direct coverage | #168, #172 |
...
| `devrel-team` | `routes` | Intentionally internal — registrar internals are not a launch workflow | Partial — API/reference and release journeys lack end-to-end checks | Partial — graph/route concepts exist without launch framing | Partial — 8 browser-oriented routes lack direct coverage | #168, #172 |
...
| `platform-docs-team` | `routes` | Intentionally internal — implementation routes are not the migration entry point | Partial — journey/dead-end validation is absent | Existing — route and graph architecture is documented | Partial — 8 browser-oriented routes still lack direct coverage | #168, #172 |
