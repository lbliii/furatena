...
|---|---|---|---|---|
| `semantic_search` | `query` | `edition`, `include_eol`, `include_preview`, `limit`, `mount`, `query`, `status`, `tag`, `url_prefix` | `query`, `ranking`, `filters`, `count`, `results` | Hybrid keyword and semantic retrieval; `limit` is 1–50 and filters are echoed in the result. |
| `retrieve_node` | `node_id` | `include_eol`, `node_id` | `node_id`, `chunks`, `backlinks`, `api_operation` | Retrieve one accessible catalog node and its context. |
| `query_graph` | none | `edition`, `edge`, `edge_kind`, `format`, `from`, `include_eol`, `include_preview`, `include_private`, `kind`, `lang`, `limit`, `link_edge`, `linked_from`, `linked_to`, `locale`, `mount`, `offset`, `owner`, `source`, `status`, `tag`, `target`, `team`, `to` | `query`, `page_count`, `edge_count`, `total`, `edge_total`, `limit`, `offset`, `next_offset`, `pages`, `edges`, `graph_nodes` | Filter and paginate pages plus their DCP edges; private inclusion is bounded by session policy. |
...
| `fura.remote_shard`, `fura.remote_shard.fetch`, `fura.remote_shard.unavailable`, `fura.remote_shard.verify` | Remote federation composition failed. For `fetch`, restore bounded same-origin HTTPS access; for `verify`, repair trust policy or upstream signed material; for `unavailable`, restore or select a verified generation. The last-known-good generation for an unrelated mount remains active. |
...
- `fura.remote_shard`, `fura.remote_shard.fetch`, `fura.remote_shard.unavailable`, `fura.remote_shard.verify`
