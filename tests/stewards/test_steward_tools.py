...
def test_generated_root_exposes_commands_and_search_contracts() -> None:
    ...
    assert "PYTHON_GIL=0" in root_map
    assert "make ci-fast" in root_map
    assert "make ci-contract" in root_map
...
def test_verifier_rejects_uncovered_source_domain(tmp_path: Path) -> None:
    source = tmp_path / "src" / "furatena" / "unmapped"
    source.mkdir(parents=True)
    (source / "feature.py").write_text("VALUE = 1\n", encoding="utf-8")
    manifest = tmp_path / "manifest.toml"
    manifest.write_text(
        """
coverage_roots = ["src/furatena"]
...
""".strip(),
    ...
    assert result.returncode == 1
    assert "uncovered code domain: src/furatena/unmapped" in result.stdout
